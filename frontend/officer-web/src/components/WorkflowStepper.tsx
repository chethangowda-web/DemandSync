...
export default function WorkflowStepper({ stages, current }: { stages: Stage[]; current: string }) {
  return (
      ...
      {stages.map(s => {
        ...
        return (
          <div key={s.key} style={{ display: 'flex', alignItems: 'center', gap: 8, background: isCurrent ? '#eff6ff' : 'transparent', padding: '6px 8px', borderRadius: 6, border: isCurrent ? '2px solid #0f2a44' : '1px solid transparent' }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: s.status === 'COMPLETE' ? '#16a34a' : s.status === 'ACTIVE' ? '#facc15' : '#cbd5e1' }} />
